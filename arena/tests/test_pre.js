...
assert.strictEqual(true, context.Module.arenaCaptureRequireAlive);
...
assert.ok(writes[1][1].includes('FLOOR_DETAIL 2'));
assert.ok(writes[1][1].includes('TEXTURE_MODE_0 -1'));
assert.ok(writes[1][1].includes('FLOOR_RED 1'));
assert.ok(!writes[1][1].includes('GRID_SIZE'));
...
assert.strictEqual(0, context.Module.arenaInputStatus.acceptedActions);
assert.strictEqual(false, context.Module.arenaInputStatus.localPlayerPresent);
assert.strictEqual(false, context.Module.arenaInputStatus.localObjectPresent);
assert.strictEqual(false, context.Module.arenaInputStatus.localObjectAlive);
assert.strictEqual(0, context.Module.arenaInputStatus.predictionCorrections);
context.Module.arenaRecordPredictionCorrection();
assert.strictEqual(1, context.Module.arenaInputStatus.predictionCorrections);
context.Module.arenaInputStatus.predictionCorrections = 0xffffffff;
context.Module.arenaRecordPredictionCorrection();
assert.strictEqual(0xffffffff, context.Module.arenaInputStatus.predictionCorrections);
